...
def neighborhood(pos_bmu,shape,width):
    ...
    @type pos_bmu: tuple
    @param shape: Taille de la SOM = V
    @type shape: tuple
    @param width: Largeur du voisinage = sigma
    @type width: float
...
def distance(proto,inp):
    ...
    @param proto: Prototypes de la SOM = W
    @type proto: numpy.array
    @param input: Entrée courante = X
...
class SOM:
    ...
    def learn(self,train_samples,epochs,lrate,width,verbose=False):
        ...
        @type epochs: int
        @param lrate: Taux d'apprentissage = Eta
        @type lrate: float
